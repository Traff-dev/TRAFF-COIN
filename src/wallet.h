...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TRAFF at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTRAFF send
enum ZerocoinSpendStatus {
    ZTRAFF_SPEND_OKAY = 0,                            // No error
    ZTRAFF_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTRAFF_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTRAFF_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTRAFF_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTRAFF_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTRAFF_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTRAFF_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTRAFF_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTRAFF_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTRAFF_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTRAFF_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTRAFF_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTRAFF_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTRAFF_SPENT_USED_ZTRAFF = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZTRAFFAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 3000 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
