...
#include "ui_zTRAFFcontroldialog.h"
...
std::list<std::string> ZTRAFFControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTRAFFControlDialog::listMints;

ZTRAFFControlDialog::ZTRAFFControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTRAFFControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTRAFFControlDialog::~ZTRAFFControlDialog()
{
    delete ui;
}

void ZTRAFFControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTRAFFControlDialog::updateList()
{
...
void ZTRAFFControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTRAFFControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTRAFF_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTRAFFControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTRAFFControlDialog::GetSelectedMints()
{
...
void ZTRAFFControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
