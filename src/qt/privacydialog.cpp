                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTRAFF ought to be enough for anybody." - Bill Gates, 2017
    ui->zTRAFFpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTRAFFSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTRAFF->setEnabled(false);
        ui->pushButtonMintzTRAFF->setToolTip(tr("zTRAFF is currently disabled due to maintenance."));

        ui->pushButtonSpendzTRAFF->setEnabled(false);
        ui->pushButtonSpendzTRAFF->setToolTip(tr("zTRAFF is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTRAFFpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTRAFF_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTRAFF is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTRAFF...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTRAFF in ") + 
...
void PrivacyDialog::on_pushButtonSpendzTRAFF_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTRAFF is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTRAFF
        sendzTRAFF();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTRAFF
    sendzTRAFF();
}

void PrivacyDialog::on_pushButtonZTRAFFControl_clicked()
{
    ZTRAFFControlDialog* zTRAFFControl = new ZTRAFFControlDialog(this);
    zTRAFFControl->setModel(walletModel);
    zTRAFFControl->exec();
}

void PrivacyDialog::setZTRAFFControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTRAFFSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTRAFF()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Traff Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTRAFFpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTRAFF is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TRAFF </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTRAFF</b>";
    ...
    // use mints from zTRAFF selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTRAFFControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTRAFFControlDialog::GetSelectedMints();
    }
    ...
    // Clear zTRAFF selector in case it was used
    ZTRAFFControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTRAFF Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TRAFF, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTRAFF Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTRAFF </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTRAFF "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTRAFF "));
    ui->labelzTRAFFAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTRAFFSyncStatus->setVisible(fShow);
}
